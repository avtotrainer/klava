...
class KeyboardTrainer:
    ...
    def draw_keyboard(self):
        ...
        for row_i, row in enumerate(KEYBOARD):
            ...
            for col_i, letter in enumerate(row):
                ...
                # ტექსტი თავიდან ყოველთვის ცარიელი
    ...
    def on_key_press(self, event):
        ...
        # შეცდომისთვის - ფერი + დროებით ასოს გამოჩენა
        if pressed in self.key_boxes and pressed != self.target_letter:
            ...
            # დროებით გამოჩნდეს ასო და ფერი
            self.canvas.itemconfig(rect_id, fill="red")
            self.canvas.itemconfig(text_id, text=pressed)

            # 200ms შემდეგ გაქრეს და დაბრუნდეს ნორმალური ფერი
            def reset():
